...
\end{itemize}
\rmk{
  Notice that in fact we can lower and raise the $ \mu, \nu $ indices
  with $ g $ and raise and lower the $ a,b $ indices with $ \eta $.
  This is totally consistent with the definition of frame fields.
}
...
\rmk{
  Note that exterior derivative is using the operator $ d $ which is
  defined using the derivative $ \partial $ not $ \nabla $.
}

Then where is the Riemann curvature tensor? It can be found in the
...
Using the vielbein formalism, we can have more redundency in
describing the spactime. This is because at each point we can choose
different orthonormal basis in the tangent space, which are related
by local Lorentz transformations.

If we use the notation of $  \partial_{\mu} = e_{\mu}{}^a e_a $ and $
e_a = e_a{}^\mu \partial_{\mu} $, then a local Lorentz transformation
can be written as:
