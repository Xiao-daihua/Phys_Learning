...
More importantly, we now can see that the Diation Operator in radial quantization is given by:
\thm{
...
Proof: we can see this easily from the conformal ward identity that it act on primary fields just generates scaling transformation. Which means it is the Diation Operator.
\rmk{
  Note that though we see this from primary fields, this is independent of the field we act on.
}
