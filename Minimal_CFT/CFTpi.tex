...
\begin{align}\label{eq:holomorphic EM tensor}
  T(z)&= -2\pi T_{zz}(z)=-\pi/2(T_{00}-T_{11}-2iT_{01})\\
  \bar{T}(\bar{z})&=-2\pi T_{\bar{z}\bar{z}}(\bar{z})=-\pi/2(T_{00}-T_{11}+2iT_{01})\\
  T_{z\bar{z}}&=\frac{1}{4}T^\mu{}_\mu
\end{align}
\YL{[I copy this from the fudan lecture note, I haven't check this carefully yet.]}
...
\begin{itemize}
  \item Total derivative in holomorphic coordinate:
    ...
    \end{align}
    \rmk{
    The integral contour is conter-clockwise for the holomorphic part and clockwise for the antiholomorphic part.(we write $ \partial M $ but it have different orientation for holomorphic and antiholomorphic part)
    }
...
\begin{align}
  LHS = \frac{1}{2}i\int_{\partial M}\left(-dz\langle T^{\bar{z}\bar{z}}\epsilon_{\bar{z}}X\rangle+d\bar{z}\langle T^{zz}\epsilon_{z}X\rangle\right)
\end{align}
...
\begin{align}
  LHS =& -\frac{1}{2\pi i}\oint_{\partial M}dz\mathrm{~}\epsilon(z)\langle T(z)X\rangle+\frac{1}{2\pi i}\oint_{\partial M}d\bar{z}\mathrm{~}\bar{\epsilon}(\bar{z})\langle\bar{T}(\bar{z})X\rangle \\ 
  =&  -\frac{1}{2\pi i}\oint_{C}dz\mathrm{~}\epsilon(z)\langle T(z)X\rangle-\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{z}\mathrm{~}\bar{\epsilon}(\bar{z})\langle\bar{T}(\bar{z})X\rangle \\ 
\end{align}
\rmk{
  Here we use that the contour $ C $ is conter-clockwise for the holomorohic part and correspondingly clockwise for the antiholomorphic part. Similarly the contour $ \bar{C} $ is conter-clockwise for the antiholomorphic part and clockwise for the holomorphic part.
}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{assets/contourexplain.png}
  \caption{Explanation of the Contours $ C $ and $ \bar{C} $}
  \label{fig:contourexplain}
\end{figure}
...
\begin{align}
  RHS=&-\frac{1}{2\pi i}\oint_{C}dw\mathrm{~}\epsilon(w)\sum_{i=1}^n\left[\frac{1}{w-z_i}\partial_{z_i}+\frac{h_i}{(w-z_i)^2}\right]\langle X\rangle\\
      &-\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{w}\mathrm{~}\bar{\epsilon}(\bar{w})\sum_{i=1}^n\left[\frac{1}{\bar{w}-\bar{z}_i}\partial_{\bar{z}_i}+\frac{\bar{h}_i}{(\bar{w}-\bar{z}_i)^2}\right]\langle X\rangle
\end{align}

\rmk{
  Note the the Residue theorem take the $ + $ sign when integrating conter-clockwise and $ - $ sign when integrating clockwise. 
  \begin{align}
    \oint_C dz f(z)=2\pi i\sum_i\mathrm{Res}(f,z_i),\quad \oint_{\bar{C}}d\bar{z} f(\bar{z})=2\pi i\sum_i\mathrm{Res}(f,\bar{z}_i) = - \oint_C d\bar{z} f(\bar{z})
  \end{align}
  thus we integrate both conter-clockwise here ($ C $ contour for the holomorphic part  and $ \bar{C} $ contour for the antiholomorphic part) to get the correct signs.
}
Thus we conbine the LHS and RHS and get the Conformal Ward Identity for Primary fields:
\thm{\label{thm:conformal ward identity primary}
  ...
  \begin{align}
    -\frac{1}{2\pi i}\oint_{C}dz\mathrm{~}\epsilon(z)\langle T(z)X\rangle-\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{z}\mathrm{~}\bar{\epsilon}(\bar{z})\langle\bar{T}(\bar{z})X\rangle &= \sum_{i=1}^n\left[-\epsilon(z_i)\partial-h_i\partial\epsilon(z_i)-\bar{\epsilon}(\bar{z}_i)\bar\partial-\bar{h}_i\bar{\partial}\bar{\epsilon}(\bar{z}_i)\right]\langle X\rangle \\ 
                                                                                                                                                                                  &= \delta_{\epsilon,\bar{\epsilon}}\langle X\rangle
  \end{align}
  note the integral contour is different for the holomorphic and antiholomorphic parts as explained above and $ \epsilon(z)^* = \bar{\epsilon}(\bar{z})  $
}
...
Thus we have the Conformal Ward Identity for Primary fields decompled into holomorphic and antiholomorphic parts by taking $ \epsilon(z) = 0 $ or $ \bar{\epsilon}(\bar{z}) = 0 $:
\thm{
  \textbf{Conformal Ward Identity for Primary Fields (decoupled form)}
...
\begin{align}
  ...
  -\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{z}\mathrm{~}\bar{\epsilon}(\bar{z})\langle \bar{T}(\bar{z})X\rangle&=-\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{w}\mathrm{~}\bar{\epsilon}(\bar{w})\sum_{i=1}^n\left[\frac{1}{\bar{w}-\bar{z}_i}\partial_{\bar{z}_i}+\frac{\bar{h}_i}{(\bar{w}-\bar{z}_i)^2}\right]\langle X\rangle
\end{align}
...
\subsubsection{Remarks on 2 Conventions of E-M Tensor}

There are two conventions of the relation between the E-M tenor and the holomorphic one. 
\begin{itemize}
  \item \textbf{Convention of the Yellow Book: } $ T(z) = -2 \pi T_{zz} $
  \item \textbf{Other Convention: } $ T(z) =  T_{zz} $
\end{itemize}
As far as I know and I think people are able to track, $ T(z) $ in both conventions are exactly the same. They all satisfy the same properties and given the same ward isentity. \textbf{Convention are only different because the different definition of the lagrangian and $ T_{\mu\nu} $ in different textbooks.}

What is different is the definition of $ T_{\mu\nu} $. However, textbooks never tell us how they defines $ T_{\mu\nu} $ is none unique. In fact we don't need to worry about this because, we will always use $ T(z) $ as a standard.



\subsection{Conformal Ward Identity}

\subsubsection{Conformal Ward Identity for General Fields}
...
\end{align}
  or if we decouple the holomorphic and antiholomorphic parts:
  \begin{align}
    \delta_{\epsilon}\langle X\rangle=-\frac{1}{2\pi i}\oint_Cdz\epsilon(z)\langle T(z)X\rangle \\ 
    \delta_{\bar{\epsilon}}\langle X\rangle=-\frac{1}{2\pi i}\oint_{\bar{C}}d\bar{z}\bar{\epsilon}(\bar{z})\langle \bar{T}(\bar{z})X\rangle
  \end{align}
}
...
\subsubsection{Remark on the convention of infinistesimal variation}

The infinitesimal variation $ \delta_{\epsilon,\bar{\epsilon}}$ has two mainly used conventions:

\begin{itemize}
  \item \textbf{Yellow book convention: } $ \delta_{\epsilon,\bar{\epsilon}} = - h\phi\partial\epsilon - \epsilon\partial\phi - \bar{h}\phi\bar{\partial}\bar{\epsilon} - \bar{\epsilon}\bar{\partial}\phi $
  \item \textbf{Other convention: } $ \delta_{\epsilon,\bar{\epsilon}} =  h\phi\partial\epsilon + \epsilon\partial\phi + \bar{h}\phi\bar{\partial}\bar{\epsilon} + \bar{\epsilon}\bar{\partial}\phi $
\end{itemize}
Using different conventions will lead to a minus sign difference in the conformal ward identity. Here we follow the yellow book convention.
...
There is another straightforward observation. OPE of the weight field should transform as the same style under scale transformation. Thus the OPE of fields should take the following general form:
\thm{
  \textbf{General Form of OPE in CFT}

  For a CFT the OPE between two general fields $ A(z,\bar{z}) $ and $ B(z,\bar{z}) $ with holomorphic and antiholomorphic dimensions $ (h_A,\bar{h}_A) $ and $ (h_B,\bar{h}_B) $ is:
  \begin{align}
    A(z_1,\bar{z}_1)\cdot B(z_2,\bar{z}_2) \sim \sum_k C_{AB}^k(z_1-z_2)^{-h_A - h_B + h_k}(\bar{z}_1-\bar{z}_2)^{-\bar{h}_A - \bar{h}_B + \bar{h}_k}O_k(z_2,\bar{z}_2)+\cdots,
  \end{align}
  Where $ C_{AB}^k $ is a constant and $ O_k(z,\bar{z}) $ is a general field with holomorphic and antiholomorphic dimensions $ (h_k,\bar{h}_k) $.
}
...
\thm{
  ...
  \begin{align}
    ...
    &=\left(\frac{dw}{dz}\right)^{-2}T(z)+\frac{c}{12}\{z;w\}
  \end{align}
