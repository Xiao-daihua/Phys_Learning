...
The Proof is quite straightforward. We note that in the middow part we have to change the order of the operators because of the definition of radial ordering. 
\rmk{
  Note that the definition of redial ordering is different for fermions, we have:
  \begin{align}
    \mathcal{R}a(z)b(w)=\begin{cases}
      a(z)b(w) & |z|>|w|\\
      -b(w)a(z) & |z|<|w|
    \end{cases}
  \end{align}
  Thus, for fermionic operators the above theorem should be modified to \textbf{changing the commutator to anti-commutator}
}
...
In canonical quantization, we define the symmetry charge as the quantization of the noether charge which is also the generator of symmetry transformation by commutators. The property of OPE between general fields \cref{axm:conformal ward identity} leads to that:
\begin{align}
...
\rmk{
  In fact this claim is not so accurate. In fact, if we only want to act on the field at $ w $, the contour should be a circle around the point $ w $. But now we just assume that the operator act on every point inside the contour $ C $, which is a equal time slice. 
}
...
\begin{align}
  T(z)=\sum_{n\in\mathbb{Z}}z^{-n-2}L_n\quad L_n=\frac{1}{2\pi i}\oint dzz^{n+1}T(z)\\
...
\begin{align}
  Q_\epsilon=\sum_{n\in\mathbb{Z}}\epsilon_nL_n \quad \epsilon(z)=\sum_{n\in\mathbb{Z}}\epsilon_nz^{n+1}
\end{align}
...
\rmk{
  ...
  \begin{itemize}
    \item The legendre transformation of Lagrangian 
      \item The integral of the time-time component of E-M Tensor on equal time slice
        \item The conserve charge of time translation symmetry
          \item Generator of time translation
  \end{itemize}
  We may find that the first three definition has subtlety, for example the lagrangian is equivalent if we add some constant term or times some constants which depends on convention. However, with a concrete definition of the infinitesimal transformation, we have a concrete generator. This definition is the most robust one. 

  Thus, in CFT, we \textbf{define} the Hamiltonian as the generator of time translation in radial quantization.
}

In a QFT the Hamiltonian is the Time Direction Generator. Thus in the content of radial quantization the Hamiltonian is the Diation Generator. If we use the result in the holomorphic coordinate, we can see that:
\thm{
...
We define the Hamiltonian as the dilation transformation generator, the dilation transformation is defined as:
\begin{align}
  z\to z'=(1+\epsilon)z \quad \bar{z}\to \bar{z}'=(1+\bar\epsilon)\bar{z}
\end{align}
Thus, the generator of this transformation is given by:
\begin{align}
  Q_{\epsilon\bar{\epsilon}} = \frac{1}{2\pi i}\oint_C dz \; \epsilon(z) \, T(z) + \frac{1}{2\pi i}\oint_{\bar{C}} d\bar z \; \bar\epsilon(\bar z) \, \bar T(\bar z) 
\end{align}
