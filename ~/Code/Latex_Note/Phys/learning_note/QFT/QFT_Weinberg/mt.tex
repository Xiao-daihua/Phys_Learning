...
其中$ E_\alpha = p^0_1+p^0_2+... $ 。\textbf{也正是因为无限小的时间平移对应的算符的本征值是能量，所以我们把它认为等同于hamiltonian}
...
\imp{In and Out states严格定义}{
  ...
  \defi{
    ...
    \begin{enumerate}
      \item 【对于$ H $来说 】和【$ \Phi_\alpha $ 对于$ H_0 $来说】有一样本征值的量子态$ \Phi_{\alpha}^{\pm} $也就是说：
        \eq{
          H\Psi_\alpha^\pm=E_\alpha\Psi_\alpha^\pm
        }
      
        注意，这里的$ E_\alpha $是和\(\Phi_\alpha\)对于\( H_0 \)的本征值一样的。
...
\YL{这里没有看，但是我感觉不是那么重要}

\section{S Matrix Lorentz Symmetry}\label{sec:SMatrix Lorentz Symmetry} % (fold)
...
\imp{Lorentz Invariant}{

  但是原则上in and out states对应的算符不见得是一样的。我们定义如果理论是具有洛伦兹不变性的话那么作用在in state 以及 out states上面的算符是完全一样的。
}
...
我们可以根据公式\cref{eq:multitrans}代入右面的式子得到$S'$和$S$矩阵之间的关系：
\begin{align}
...
\textbf{1. 寻找Lorantz Invariant的等价条件}
...
\imp{Exact Generators}{
  对于一个有相互作用的理论。我们存在Hamiltonian是$H= H_0+V$。对于$H_0$理论来说，我们存在一系列生成元$H_0,P_0,J_0,K_0$。保证free particle states 满足洛伦兹变换关系\cref{eq:multitrans}。

现在我们定义另一套全新的生成元$H,P,J,K$，使得其\textbf{作用在in state上面}依然满足\cref{eq:multitrans}关系。我们称这套生成元为Exact Generators。

\hlr{注意！！我们只定义作用在in states上面！只有lorentz Invariant的理论作用在out states上面也是一样的！！}
}
...
对于Exact Generators来说，我们也有类似的对易关系:
\begin{equation}
  \begin{aligned}
...
\imp{一般相互作用理论}{
  下面是一个事实，就是对于一般相互作用理论我们会发现。如果我们寻找一个让in state按照\cref{eq:multitrans}变换的Exact Generators的话。这个exact generator会满足：
  \begin{equation}
    H=H_0+V\quad,\quad\mathbf{P}=\mathbf{P}_0\quad,\quad\mathbf{J}=\mathbf{J}_0.
    \label{eq:normalgeneratorexact}
  \end{equation}
}
...
\YL{具体的细节看看Weinberg怎么写的捏}

\textbf{4. 直接证明一般的量子场论理论in and out states作用unitary算符是一样的}
...
% section SMatrix Lorentz Symmetry (end)



\section{S Matrix Internal Symmetry}\label{sec:S Matrix Internal Symmetry} % (fold)
...
% section S Matrix Internal Symmetry (end)
